...
class NftGenesis {
  constructor (localConfig = {}) {
    ...
    if (!localConfig.adapters) {
      throw new Error('Adapters are required for the nft-genesis.js use case library.')
    }
  ...
  async processTx (data) {
    try {
      ...
      if (!inputsAreValid) {
        ...
        await this.adapters.txDb.createTx(txid, data.txData)
  ...
  async validateInputs (data) {
    try {
      ...
      const groupInfo = await this.adapters.cache.get(groupId)
  ...
  async addTokenToDB (data, groupTokenId) {
    try {
      ...
      const groupToken = await this.adapters.tokenDb.getToken(groupTokenId)
      if (Array.isArray(groupToken.nfts)) {
        groupToken.nfts.push(slpData.tokenId)
        await this.adapters.tokenDb.createToken(groupToken.tokenId, groupToken)
      }

...
import ZMQ from '../../../src/adapters/zmq.js'
...
describe('#zmq.js', () => {
  ...
  describe('#monitorZmq', () => {
    it('should process messages from the socket', async () => {
      const topic = Buffer.from(mockData.topic01, 'hex')
      const message = Buffer.from(mockData.msg01, 'hex')

      // Create an async generator that yields one message then throws to exit the loop
      uut.sock = {
      ...
      sandbox.stub(uut, 'decodeMsg').returns(true)

      try {
        await uut.monitorZmq()
        assert.fail('Unexpected code path')
      } catch (err) {
        // Expected to throw 'test break' after processing the message
        assert.equal(err.message, 'test break')
        // Verify decodeMsg was called with the correct arguments
        assert.equal(uut.decodeMsg.called, true)
        assert.equal(uut.decodeMsg.calledWith(topic, message), true)
      }
    })

    it('should catch and throw errors', async () => {
      // Create a socket that throws an error immediately
      uut.sock = {
        async * [Symbol.asyncIterator] () {
